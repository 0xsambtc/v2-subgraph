...
function isCompleteMint(mintId: string): boolean {
  ...
  return mintEvent ? mintEvent.sender !== null : false // sufficient checks
}
...
export function handleTransfer(event: Transfer): void {
  ...
  if (factory === null) {
    log.error('Factory is null', [FACTORY_ADDRESS])
    return
  }
  ...
  if (pair === null) {
    log.error('Pair is null', [event.address.toHexString()])
    return
  }
...
export function handleSync(event: Sync): void {
  ...
  if (token0 === null) {
    log.error('Token0 is null', [pair.token0])
    return
  }
  let token1 = Token.load(pair.token1)
  if (token1 === null) {
    log.error('Token1 is null', [pair.token1])
    return
  }
  let uniswap = UniswapFactory.load(FACTORY_ADDRESS)
  if (uniswap === null) {
    log.error('UniswapFactory is null', [FACTORY_ADDRESS])
    return
  }
...
export function handleMint(event: Mint): void {
  ...
  let mint = MintEvent.load(mints[mints.length - 1])
  if (mint === null) {
    log.error("Mint is null", [mints[mints.length - 1]])
    return
  }
...
export function handleBurn(event: Burn): void {
  ...
  if (burn === null) {
    log.error("Burn is null", [burns[burns.length - 1]])
    return
  }
...
export function handleSwap(event: Swap): void {
  ...
  let uniswap = UniswapFactory.load(FACTORY_ADDRESS)
  if (uniswap === null) {
    throw new Error(`UniswapFactory ${FACTORY_ADDRESS} is null`)
  }
